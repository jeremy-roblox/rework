...
from ..constants import SHARD_RANGE, CLUSTER_ID, SHARD_COUNT, RELEASE, SELF_HOST, PLAYING_STATUS
from ..secrets import (REDIS_CONNECTION_STRING, MONGO_CONNECTION_STRING, MONGO_CA_FILE, DISCORD_PROXY)
...
LABEL = env.get("LABEL", "Howblox")
SHARD_SLEEP_TIME = int(env.get("SHARD_SLEEP_TIME", "5"))

logger = logging.getLogger()


class HowbloxStructure(AutoShardedClient):
    ...
    def get_database():
        ...
        db = motor.motor_asyncio.AsyncIOMotorClient(MONGO_CONNECTION_STRING, tlsCAFile="src/cert.crt" if MONGO_CA_FILE else None)["howblox"]
    ...
    def module(module):
        ...
        Howblox.log(f"Loaded {module_name}")

        if hasattr(new_module, "__loaded__"):
            asyncio.run_coroutine_threadsafe(new_module.__loaded__(), loop)

        if HowbloxStructure.loaded_modules.get(module_dir):
            HowbloxStructure.loaded_modules[module_dir][module_name] = new_module
        else:
            HowbloxStructure.loaded_modules[module_dir] = {module_name: new_module}
    ...
    def loader(module):
        ...
        #HowbloxStructure.loaded_modules[module.__name__.lower()] = [load, stuff]
    ...
    def get_module(dir_name, *, name_override=None, name_override_pattern="", path="resources.modules", attrs=None):
        ...
        modules = HowbloxStructure.loaded_modules.get(save_as)
        ...
        if not modules:
            ...
            try:
                ...
                Howblox.log(f"ERROR | {e}")
                traceback_text = traceback.format_exc()
                traceback_text = len(traceback_text) < 500 and traceback_text or f"...{traceback_text[len(traceback_text)-500:]}"
                Howblox.error(traceback_text, title=f"{dir_name}.py")

            except Exception as e:
                Howblox.log(f"ERROR | Module {dir_name} failed to load: {e}")
                traceback_text = traceback.format_exc()
                traceback_text = len(traceback_text) < 500 and traceback_text or f"...{traceback_text[len(traceback_text)-500:]}"
                Howblox.error(traceback_text, title=f"{dir_name}.py")
            else:
    ...
    def command(*args, **kwargs):
        return Howblox.get_module("commands", attrs="new_command", name_override_pattern="Command_")(*args, **kwargs)

    @staticmethod
    def extension(*args, **kwargs):
        return Howblox.get_module("commands", attrs="new_extension", name_override_pattern="Extension_")(*args, **kwargs)
